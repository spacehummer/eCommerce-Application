const enum TextContentEnum {
  PLACEHOLDER = 'Placeholder',
  PLACEHOLDER_1 = 'Placeholder_1',
  PLACEHOLDER_2 = 'Placeholder_2',
  GENERAL_LOGO_NAME = 'BookSavior',
  NAV_MENU_SIGN_IN = 'Sign in',
  /* Section: welcome */
  MAIN_WELCOME_SECTION_HEADING = 'Welcome to BookSavior!',
  MAIN_WELCOME_SECTION_TB1_P1 = 'Our service connects people who appreciate books and people who have unclaimed books.',
  MAIN_WELCOME_SECTION_TB1_P2 = 'If you have accumulated unwanted books, we can give them a second life!',
  MAIN_WELCOME_SECTION_TB1_P3 = 'If you are looking for a book to complement your library or collection - you are also in the right place!',
  /* Modal window */
  MODAL_WINDOW_BTN_CLOSE_PLACEHOLDER = 'X',
  /* # Login */
  MODAL_WINDOW_LOGIN_SUCCESSFUL_HEADING = 'Successful login',
  MODAL_WINDOW_LOGIN_SUCCESSFUL_CONTENT = 'You have successfully logged into your account!',
  MODAL_WINDOW_LOGIN_ERROR_HEADING = 'Login ERROR',
  MODAL_WINDOW_LOGIN_ERROR_CONTENT = 'Login error: invalid password or non-existent username.',
  /* # Registration */
  MODAL_WINDOW_REGISTRATION_SUCCESSFUL_HEADING = 'Successful registration',
  MODAL_WINDOW_REGISTRATION_SUCCESSFUL_CONTENT = 'Your profile has been successfully created!',
  MODAL_WINDOW_REGISTRATION_ERROR_HEADING = 'Registration ERROR',
  MODAL_WINDOW_REGISTRATION_ERROR_CONTENT = 'Error in the process of creating your profile.',
  /* Section: ERROR 404 */
  SECTION_ERROR_404_HEADING = 'ERROR 404',
  SECTION_ERROR_404_TB1_P1 = 'We can`t find the page you`re looking for.',
  SECTION_ERROR_404_TB1_P2 = 'You may have entered the wrong URL or you do not have rights to view the page.',
}

export default TextContentEnum;
